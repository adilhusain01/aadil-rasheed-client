...
console.log(
  `[API Debug] NEXT_PUBLIC_API_URL env: ${process.env.NEXT_PUBLIC_API_URL}`
);
...
const isBuildTime = () => {
  ...
  const isBuild =
    ...
    (typeof window === "undefined" &&
      ...
      Boolean(process.env.GENERATING_STATIC_PARAMS));
...
export async function fetchBlogPosts(): Promise<BlogPost[]> {
  console.log(
    `[API Debug] fetchBlogPosts() - Starting request to ${API_URL}/blog`
  );

  // During build time, return empty data to avoid API calls
  if (isBuildTime()) {
    console.log(
      "[API Debug] Build time detected, returning mock data for blog posts"
    );
  ...
  try {
    ...
    if (!Array.isArray(data.data)) {
      console.error(
        `[API Debug] Expected array but got:`,
        typeof data.data,
        data.data
      );
      return [];
...
export async function fetchBlogPostBySlug(slug: string): Promise<BlogPost> {
  ...
  console.log(
    `[API Debug] fetchBlogPostBySlug() - Starting request to ${API_URL}/blog/${encodedSlug}`
  );

  // Log information about the request for debugging
  console.log(
    `[API Debug] fetchBlogPostBySlug processing slug: '${slug}' (encoded: '${encodedSlug}')`
  );
  ...
  if (isBuildTime() || process.env.NODE_ENV === "development") {
    console.log(
      `[API Debug] Build time or development environment detected, returning mock data for blog post with slug: ${slug}`
    );
    ...
    if (slug === "ham-pe-laazim-hai-ki-ham-waqt-ko-zaaya-na-karein") {
      return {
        ...
        content:
          "<p>This is detailed mock content for this specific Urdu blog post.</p>",
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      try {
        console.log(
          `[API Debug] Fetching from: ${API_URL}/blog/${encodedSlug} (Attempt ${
            attempts + 1
          }/${maxAttempts})`
        );
    ...
    // All attempts failed
    console.error(
      `[API Debug] All ${maxAttempts} attempts failed for slug ${slug}`
    );

    // For a production environment, consider providing fallback content rather than throwing
    if (process.env.NODE_ENV === "production") {
      console.log(
        "[API Debug] Production environment detected, returning fallback content"
      );
      ...
      return {
        ...
        content:
          "<p>This blog post is available on request. Please check back later.</p>",
        date: new Date().toISOString(),
        likes: 0,
        isPublished: true,
        tags: [],
    ...
    console.error(
      `[API Debug] Error fetching blog post with slug ${slug}:`,
      error
    );
    ...
    if (process.env.NODE_ENV === "production") {
      return {
        ...
        excerpt: "Content temporarily unavailable",
        content:
          "<p>We apologize, but this content is temporarily unavailable. Please check back later.</p>",
        date: new Date().toISOString(),
        likes: 0,
        isPublished: true,
        tags: [],
...
export async function likeBlogPost(id: string): Promise<BlogPost> {
  console.log(
    `[API Debug] likeBlogPost() - Starting request to ${API_URL}/blog/${id}/like`
  );
...
export async function fetchSocialMediaLinks(): Promise<SocialMediaLink[]> {
  console.log(
    `[API Debug] fetchSocialMediaLinks() - Starting request to ${API_URL}/social`
  );

  // During build time, return empty data
  if (isBuildTime()) {
    console.log(
      "[API Debug] Build time detected, returning mock data for social links"
    );
...
export async function fetchGalleryImages(): Promise<GalleryImage[]> {
  console.log(
    `[API Debug] fetchGalleryImages() - Starting request to ${API_URL}/gallery`
  );

  // During build time, return empty data
  if (isBuildTime()) {
    console.log(
      "[API Debug] Build time detected, returning mock data for gallery images"
    );
...
export async function submitContactForm(
  ...
  console.log(
    `[API Debug] submitContactForm() - Starting request to ${API_URL}/contact`
  );
...
export async function submitSubscription(
  ...
  console.log(
    `[API Debug] submitSubscription() - Starting request to ${API_URL}/subscription`
  );
...
export async function uploadImage(file: File): Promise<{ imageUrl: string }> {
  console.log(
    `[API Debug] uploadImage() - Starting request to ${API_URL}/upload`
  );
...
export async function fetchBlogComments(blogId: string): Promise<Comment[]> {
  console.log(
    `[API Debug] fetchBlogComments() - Starting request to ${API_URL}/blog/${blogId}/comments`
  );

  try {
    console.log(
      `[API Debug] Fetching from: ${API_URL}/blog/${blogId}/comments`
    );
...
export async function submitComment(
  ...
  console.log(
    `[API Debug] submitComment() - Starting request to ${API_URL}/blog/${blogId}/comments`
  );
...
export async function submitReply(
  ...
  console.log(
    `[API Debug] submitReply() - Starting request to ${API_URL}/blog/comments/${commentId}/replies`
  );

  try {
    console.log(
      `[API Debug] Fetching from: ${API_URL}/blog/comments/${commentId}/replies`
    );
    const data = await safeFetch(
      `${API_URL}/blog/comments/${commentId}/replies`,
      {
        method: "POST",
        body: JSON.stringify({
          ...replyData,
